...
#define match_string(cs1, s2) \
  ({ size_t len = strlen (cs1);						      \
     int result = strncasecmp_l ((cs1), (s2), len, 0) == 0;	      \
     if (result) (s2) += len;						      \
     result; })
#else
        ...
        for (cnt = 0; cnt < 7; ++cnt) {
          ...
          if (s.decided != raw) {
            ...
            if (match_string(_NL_CURRENT(LC_TIME, DAY_1 + cnt), trp)
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, DAY_1 + cnt), weekday_name[cnt]))
                decided_longest = loc;
            }
            trp = rp;
            if (match_string(_NL_CURRENT(LC_TIME, ABDAY_1 + cnt), trp)
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, ABDAY_1 + cnt), ab_weekday_name[cnt]))
                decided_longest = loc;
            }
          }
#endif
        ...
        for (cnt = 0; cnt < 12; ++cnt) {
          ...
          if (s.decided != raw) {
            ...
            if (match_string(_NL_CURRENT(LC_TIME, MON_1 + cnt), trp)
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, MON_1 + cnt), month_name[cnt]))
                decided_longest = loc;
            }
            trp = rp;
            if (match_string(_NL_CURRENT(LC_TIME, ABMON_1 + cnt), trp)
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, ABMON_1 + cnt), ab_month_name[cnt]))
                decided_longest = loc;
            }
#ifdef _LIBC
            ...
            if (match_string(_NL_CURRENT(LC_TIME, ALTMON_1 + cnt), trp)
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, ALTMON_1 + cnt), alt_month_name[cnt]))
                decided_longest = loc;
            }
            trp = rp;
            if (match_string(_NL_CURRENT(LC_TIME, _NL_ABALTMON_1 + cnt),
              ...
              rp_longest = trp;
              cnt_longest = cnt;
              if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, _NL_ABALTMON_1 + cnt), alt_month_name[cnt]))
                decided_longest = loc;
            }
#endif
          }
      ...
      case 'c':
        ...
        if (s.decided != raw) {
          if (!recursive(_NL_CURRENT(LC_TIME, D_T_FMT))) {
            ...
            if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, D_T_FMT), HERE_D_T_FMT))
              s.decided = loc;
            s.want_xday = 1;
            break;
          }
      ...
      case 'x':
        ...
        if (s.decided != raw) {
          if (!recursive(_NL_CURRENT(LC_TIME, D_FMT))) {
            ...
            if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, D_FMT), HERE_D_FMT))
              s.decided = loc;
            s.want_xday = 1;
            break;
          }
      ...
      case 'r':
        ...
        if (s.decided != raw) {
          if (!recursive(_NL_CURRENT(LC_TIME, T_FMT_AMPM))) {
            ...
            if (s.decided == not &&strcmp(_NL_CURRENT(LC_TIME, T_FMT_AMPM), HERE_T_FMT_AMPM))
              s.decided = loc;
            break;
          }
          s.decided = raw;
